...
public class PhotoProcessor {
    ...
    private CelebTfLiteClassifier celebTfLiteClassifier;
    ...
    private void loadModels() {
        ...
        try {
            Log.i(TAG, "~~~~~~~~~~~~~~~ Loading CelebTfLiteClassifier ~~~~~~~~~~~~~~~~");
            celebTfLiteClassifier = new CelebTfLiteClassifier(context);
            Log.i(TAG, "~~~~~~~~~~~~~~~ End Loading CelebTfLiteClassifier ~~~~~~~~~~~~~~~~");
        } catch (IOException e) {
            Log.e(TAG, "Failed to load CelebTfLiteClassifier.", e);
        }
    }
...
//    }
    ...
    private synchronized CelebFaceData classifyFace(Bitmap bmp, StringBuilder text) {
        long startTime = SystemClock.uptimeMillis();

        Bitmap resultBitmap = Bitmap.createScaledBitmap(bmp, celebTfLiteClassifier.getImageSizeX(), celebTfLiteClassifier.getImageSizeY(), false);
        String h = String.valueOf(resultBitmap.getHeight());
        String w = String.valueOf(resultBitmap.getWidth());
        Log.i(TAG, String.format("h: %s, w: %s", h, w));

        CelebFaceData result = (CelebFaceData) celebTfLiteClassifier.classifyFrame(resultBitmap);
        long sceneTimeCost = SystemClock.uptimeMillis() - startTime;
        Log.i(TAG, "Timecost to run celeb face model inference: " + Long.toString(sceneTimeCost));
        text.append("Face attrs:").append(sceneTimeCost).append(" ms\n");
        return result;
    }
...
//        SceneData scene = classifyScenes(bmp, text);
    ...
    public ImageAnalysisResults getImageAnalysisResults(String filename, Bitmap bmp, StringBuilder text,boolean needScene)
    {
        ...
        SceneData scene=null;
        CelebFaceData attributes = null;
        if (!scenes.containsKey(key)) {
            if(needScene) {
                if (bmp == null)
                    bmp = loadBitmap(filename);
//                scene = classifyScenes(bmp, text);
                attributes = classifyFace(bmp, text);
